...
public class KeycloakAdminServiceImpl implements KeycloakAdminService {
    ...
    public UUID createUser(String email, String password, String name) {
        ...
        try {
            ...
            // Normalize to lowercase: Keycloak usernames are case-sensitive at creation but
            // may resolve differently at login time. Always store lowercase to guarantee
            // that login("user@gmail.com") matches the stored username "user@gmail.com".
    ...
    public void activateUser(UUID userId) {
        ...
        try {
            ...
            // Set all three fields before the PUT
            userRep.setEnabled(true);
            userRep.setEmailVerified(true);
            userRep.setRequiredActions(java.util.Collections.emptyList());

            // Single PUT — atomic, no race condition
            userResource.update(userRep);

            log.info("Successfully activated user (enabled=true, emailVerified=true, requiredActions=[]): userId={}", userId);

        } catch (NotFoundException e) {
            log.error("User not found in Keycloak during activation: userId={}", userId, e);
            throw new com.event.tickets.exceptions.KeycloakUserUpdateException(
                    String.format("User not found in Keycloak: %s", userId), e
            );
        } catch (Exception e) {
            log.error("Failed to activate user in Keycloak: userId={}", userId, e);
            throw new com.event.tickets.exceptions.KeycloakUserUpdateException(
                    String.format("Failed to activate user in Keycloak: %s", e.getMessage()), e
            );
        }
    }
