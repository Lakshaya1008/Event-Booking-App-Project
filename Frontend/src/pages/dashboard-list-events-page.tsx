...
const DashboardListEventsPage: React.FC = () => {
  ...
  const [sort] = useState<string | undefined>("start,desc");
  ...
  }, [isLoading, user, page, size, sort]);

  const refreshEvents = async (accessToken: string) => {
    try {
      setIsFetching(true);
      setEvents(await listEvents(accessToken, page, size, sort));
    } catch (err) {
          ...
          {events?.content.map((eventItem) => (
            <Card className="bg-gray-900 border-gray-700 text-white">
              ...
              <CardFooter className="flex justify-end gap-2">
                <Link to={`/dashboard/events/${eventItem.id}/sales-dashboard`}>
                  <Button
                    type="button"
                ...
                <Link to={`/dashboard/events/${eventItem.id}/attendees-report`}>
                  <Button
                    type="button"
