...
import { createEvent, getEvent, updateEvent, createTicketType, updateTicketType, deleteTicketType } from "@/lib/api";
...
const DashboardManageEventPage: React.FC = () => {
  ...
  const handleSaveTicketType = async () => {
    ...
    // If editing an existing event, persist via Ticket Type APIs
    if (isEditMode && eventData.id && !isLoading && user?.access_token) {
      try {
        setError(undefined);
        if (!currentTicketType.id || isTempId(currentTicketType.id)) {
          await createTicketType(user.access_token, eventData.id, {
            name: currentTicketType.name,
            price: currentTicketType.price,
            description: currentTicketType.description,
            totalAvailable: currentTicketType.totalAvailable,
          });
        } else {
        ...
        // Refresh event details to reflect server state
        const refreshed: EventDetails = await getEvent(user.access_token, eventData.id);
        updateField(
          "ticketTypes",
          refreshed.ticketTypes.map((t) => ({
            id: t.id,
            name: t.name,
            price: t.price,
  ...
  const handleDeleteTicketType = async (id: string | undefined) => {
    if (!id) {
      return;
    }

    // If editing an existing event and this is a persisted ticket type, delete via API
    if (isEditMode && eventData.id && !isTempId(id) && user?.access_token) {
      try {
        setError(undefined);
        await deleteTicketType(user.access_token, eventData.id, id);
    ...
    updateField("ticketTypes", eventData.ticketTypes.filter((t) => t.id !== id));
  };
